/* CYD Deck: ESP32-2432S028 Stream Deck. Configure TFT_eSPI for your CYD before compiling.
   Required libraries: LVGL, TFT_eSPI, XPT2046_Touchscreen, NimBLE-Arduino,
   ArduinoJson, LittleFS, PNGdec. All persistent deck data is /config.json. */
#include <Arduino.h>
#include <TFT_eSPI.h>
...
bool touchHomeButton(int x, int y) {
  ...
  if (x >= 260 && x <= 316 && y >= 2 && y <= 26) {
    folderDepth--;
    String target = pageStack[folderDepth];
    JsonArray pages = deck["profiles"][deck["activeProfile"] | 0]["pages"];
    for (uint8_t i = 0; i < pages.size(); i++) {
      if (target == String(pages[i]["name"] | "")) { pageIndex = i; drawDeck(); return true; }
    }
    pageIndex = 0; drawDeck(); return true;
  }
  return false;
}
...
void sendKey(uint8_t modifier,uint8_t key){ if(!keyboardInput)return; uint8_t report[8]={modifier,0,key,0,0,0,0,0}; keyboardInput->setValue(report,sizeof(report)); keyboardInput->notify(); delay(4); uint8_t released[8]={0}; keyboardInput->setValue(released,sizeof(released)); keyboardInput->notify(); delay(4); }
void typeText(const String& text){ for(size_t i=0;i<text.length();i++){ char c=text[i]; uint8_t mod=0,key=0; if(c>='a'&&c<='z')key=0x04 + c - 'a'; else if(c>='A'&&c<='Z'){key=0x04 + c - 'A';mod=0x02;} else if(c>='1'&&c<='9')key=0x1E + c - '1'; else if(c=='0')key=0x27; else { switch(c){ case ' ':key=0x2C;break; case '.':key=0x37;break; case '\\':key=0x31;break; case '/':key=0x38;break; case '-':key=0x2D;break; case '_':key=0x2D;mod=0x02;break; case ':':key=0x33;mod=0x02;break; case ';':key=0x33;break; case '(':key=0x26;mod=0x02;break; case ')':key=0x27;mod=0x02;break; default:continue; }} sendKey(mod,key); }}
void openOnWindows(const String& target){ if(target.length()==0)return; sendKey(0x08,0x15); delay(120); typeText(target); sendKey(0,0x28); }
void executeButton(JsonObject b){ String type=b["type"]|"", action=b["action"]|""; if(type=="Folder"){ String target=b["folder"]|""; JsonArray pages=deck["profiles"][deck["activeProfile"] | 0]["pages"]; for(uint8_t i=0;i<pages.size();i++)if(target==String(pages[i]["name"]|"")){ pageStack[folderDepth++]=String(currentPage()["name"]|""); pageIndex=i; drawDeck(); return; } }
 if(type=="Application"||type=="Open File"||type=="Open Folder"||type=="Website") openOnWindows(action);
 if(b["toggle"]|false){ bool state=b["state"]|false; b["state"]=!state; } // Host applications receive actions through BLE in future protocol extension.
 // Actions intentionally stay declarative on CYD; the Windows host performs OS-specific shortcuts.
 drawDeck(); }
