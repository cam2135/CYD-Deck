...
TYPES = ["Application", "Website", "Keyboard Shortcut", "Open File", "Open Folder", "Folder"]

def button(name="New Button"):
    return {"id": str(uuid.uuid4()), "name": name, "type": "Application", "action": "", "icon": "", "toggle": False, "toggleName": "", "toggleAction": "", "folder": ""}
...
_KEY_DISPLAY_NAMES = {
    ...
    "Prior": "PgUp", "Next": "PgDn", "Caps_Lock": "CapsLock",
    "Num_Lock": "NumLock", "Scroll_Lock": "ScrollLock", "Print": "PrintScreen",
    "KP_0": "Numpad0", "KP_1": "Numpad1", "KP_2": "Numpad2", "KP_3": "Numpad3",
    "KP_4": "Numpad4", "KP_5": "Numpad5", "KP_6": "Numpad6", "KP_7": "Numpad7",
    "KP_8": "Numpad8", "KP_9": "Numpad9", "KP_Add": "Numpad+",
    "KP_Subtract": "Numpad-", "KP_Multiply": "Numpad*", "KP_Divide": "Numpad/",
    "KP_Decimal": "Numpad.", "KP_Enter": "NumpadEnter",
}
...
class _ShortcutHandlers:
    ...
    def __init__(self, state: _ShortcutRecorderState, status_label, button_data, editor, win):
        self._state = state
    ...
    def _add_value(self, value: str) -> None:
        ...
        self._status.configure(
            text="Recorded: " + ", ".join(self._state.values)
            + "\nPress Esc, then Tab, to save it."
        )

    def _display_name(self, event) -> str:
        key = _KEY_DISPLAY_NAMES.get(event.keysym,
    ...
    def _finish(self) -> None:
        ...
        self._win.grab_release(); self._win.destroy(); self._editor.redraw()
...
class Editor(ctk.CTk):
    ...
    def snapshot(self):
        self.undo_stack.append(copy.deepcopy(self.data)); self.undo_stack = self.undo_stack[-40:]
        self.redo_stack.clear(); self._dirty = True; self._update_title()

    def undo(self):
        if self.undo_stack:
            self.redo_stack.append(copy.deepcopy(self.data)); self.data = self.undo_stack.pop()
            self.page = 0; self.selected = None; self._dirty = True; self._update_title(); self.redraw()

    def redo(self):
        if self.redo_stack:
            self.undo_stack.append(copy.deepcopy(self.data)); self.data = self.redo_stack.pop()
            self.page = 0; self.selected = None; self._dirty = True; self._update_title(); self.redraw()

    def make_ui(self):
        bar = ctk.CTkFrame(self, corner_radius=0); bar.pack(fill="x")
        toolbar_items = [
            ("New", self.new), ("Open", self.load), ("Save", self.save),
            ("Save As", self.save_as), ("Write SD card", self.export_sd),
    ...
    def draw_sidebar(self):
        ctk.CTkLabel(self.side, text="PROFILE",
            font=ctk.CTkFont(size=13, weight="bold")).pack(pady=(14, 5))
        ctk.CTkButton(self.side, text=self.profile["name"] + "  ▾",
            command=self.profile_menu).pack(fill="x", padx=10)
        ctk.CTkLabel(self.side, text="PAGES",
            font=ctk.CTkFont(size=13, weight="bold")).pack(pady=(22, 5))
        ...
        for i, p in enumerate(self.profile["pages"]):
            ...
            ctk.CTkButton(row, text=p["name"],
                fg_color="#3b8ed0" if i == self.page else "transparent",
                anchor="w", height=28,
                command=lambda x=i: self.select_page(x)).pack(
                    side="left", fill="x", expand=True, padx=(4, 2))
        ctk.CTkButton(self.side, text="+ Add page",
            command=self.add_page).pack(fill="x", padx=10, pady=(4, 8))
        ctk.CTkButton(self.side, text="Settings",
            command=self.settings).pack(side="bottom", fill="x", padx=10, pady=12)
    ...
    def _draw_grid_header(self):
        ...
        if parent_idx is not None:
            ctk.CTkButton(head, text="← Back", width=70,
                command=lambda: self.select_page(parent_idx)).pack(side="left", padx=(0, 8))
        ctk.CTkLabel(head, text=self.current_page["name"],
            font=ctk.CTkFont(size=22, weight="bold")).pack(side="left")
        ctk.CTkButton(head, text="Import app / shortcut", width=145,
            command=self.import_apps).pack(side="right", padx=(6, 0))
        ctk.CTkButton(head, text="+ Folder", width=90,
            command=self.add_folder_button).pack(side="right", padx=(6, 0))
        at_cap = len(self.current_page["buttons"]) >= 8
        ctk.CTkButton(head, text="+ Button", width=100, command=self.add_button,
            state="disabled" if at_cap else "normal").pack(side="right")
        if at_cap:
            ctk.CTkLabel(head, text="(page full — 8 max)", text_color="#888",
    ...
    def _draw_grid_buttons(self):
        ...
        items = [b for b in self.current_page["buttons"]
    ...
    def _draw_button_card(self, grid, i, b):
        ...
        f = ctk.CTkFrame(grid, corner_radius=14, border_width=2, border_color=border_color)
        ...
        ctk.CTkLabel(f, text=icon_text, text_color=type_color,
            font=ctk.CTkFont(size=14)).pack(expand=True)
        if is_folder and b.get("folder"):
            ctk.CTkLabel(f, text=f'→ {b["folder"]}', text_color="#7aadcc",
                font=ctk.CTkFont(size=11)).pack()
        lbl = ctk.CTkLabel(f, text=b["name"], wraplength=120,
            font=ctk.CTkFont(size=16, weight="bold"))
        lbl.pack(pady=(0, 10))
        lbl.bind("<Button-1>", lambda e, x=b: self.select(x))
        if is_folder:
            ctk.CTkLabel(f, text="double-click to open", text_color="#555",
                font=ctk.CTkFont(size=10)).pack(pady=(0, 6))
    ...
    def draw_inspector(self):
        if not self.selected:
            ctk.CTkLabel(self.inspector, text="Select a button to edit it.",
                wraplength=220).pack(pady=25)
            return
        b = self.selected
        ctk.CTkLabel(self.inspector, text="BUTTON",
            font=ctk.CTkFont(weight="bold")).pack(pady=(14, 4))
        ...
        ctk.CTkButton(self.inspector, text="Duplicate",
            command=self.duplicate).pack(fill="x", padx=10, pady=4)
        ctk.CTkButton(self.inspector, text="Delete", fg_color="#a33",
            hover_color="#822", command=self.delete).pack(fill="x", padx=10, pady=4)
    ...
    def _draw_toggle_section(self, b):
        ...
        def set_toggle():
            if b.get("toggle") != v.get():
                self.snapshot()
                b["toggle"] = v.get()
        ctk.CTkCheckBox(self.inspector, text="Smart toggle", variable=v,
            command=set_toggle).pack(anchor="w", padx=12, pady=8)
    ...
    def _draw_folder_inspector(self, b):
        ...
        def on_pick(choice):
            ...
            if b.get("folder") != value:
                self.snapshot(); b["folder"] = value; self.redraw()
    ...
    def record_shortcut(self, b):
        ...
        win.title("Record shortcut"); win.geometry("390x180"); win.resizable(False, False)
        ctk.CTkLabel(win, text="Press the shortcut you want to use",
            font=ctk.CTkFont(size=17, weight="bold")).pack(pady=(26, 8))
        status = ctk.CTkLabel(win,
        ...
        ctk.CTkButton(win, text="Cancel",
            command=lambda: (win.grab_release(), win.destroy())).pack(side="bottom", pady=18)
    ...
    def field(self, label, obj, key):
        ...
        e = ctk.CTkEntry(self.inspector); e.insert(0, obj.get(key, "")); e.pack(fill="x", padx=12)
        def commit(w=e, o=obj, k=key):
            val = w.get()
            if o.get(k) != val:
                self.snapshot(); o[k] = val; self.redraw()
    ...
    def choice(self, label, obj, key, values):
        ctk.CTkLabel(self.inspector, text=label, anchor="w").pack(fill="x", padx=12, pady=(9,0))
        def on_change(x): self.snapshot(); obj[key] = x; self.redraw()
        ctk.CTkOptionMenu(self.inspector, values=values, variable=tk.StringVar(value=obj[key]), command=on_change).pack(fill="x", padx=12)
    ...
    def _page_menu(self, idx):
        ...
        win.bind("<FocusOut>", lambda e: close()); win.focus_force()

    def _page_menu_buttons(self, win, idx, pages, action):
        cfg = dict(anchor="w", fg_color="transparent", hover_color="#3a3a3a", corner_radius=4)
        ctk.CTkButton(win, text="✏  Rename", width=160, **cfg,
            command=lambda: action(lambda: self._rename_page(idx))).pack(fill="x", padx=4, pady=(4, 1))
        ctk.CTkButton(win, text="↑  Move up", width=160, **cfg,
            state="normal" if idx > 0 else "disabled",
            command=lambda: action(lambda: self._move_page(idx, -1))).pack(fill="x", padx=4, pady=1)
        ctk.CTkButton(win, text="↓  Move down", width=160, **cfg,
    ...
    def _rename_page(self, idx):
        pages = self.profile["pages"]
        old = pages[idx]["name"]
        new = simpledialog.askstring("Rename page", "New name:", initialvalue=old, parent=self)
    ...
    def _move_page(self, idx, direction):
        pages = self.profile["pages"]; new_idx = idx + direction
        if new_idx < 0 or new_idx >= len(pages): return
        self.snapshot(); pages[idx], pages[new_idx] = pages[new_idx], pages[idx]
        if self.page == idx: self.page = new_idx
        elif self.page == new_idx: self.page = idx
        self.redraw()

    def _delete_page(self, idx):
        pages = self.profile["pages"]
        if len(pages) <= 1: messagebox.showinfo("Cannot delete", "You need at least one page."); return
        if not messagebox.askyesno("Delete page", f'Delete "{pages[idx]["name"]}" and all its buttons?'): return
        self.snapshot(); pages.pop(idx); self.page = min(self.page, len(pages)-1); self.selected = None; self.redraw()

    def add_button(self):
        if len(self.current_page["buttons"]) >= 8:
            messagebox.showinfo("Page full", "A page holds a maximum of 8 buttons."); return
        self.snapshot(); b = button(f"Button {len(self.current_page['buttons'])+1}"); self.current_page["buttons"].append(b); self.selected = b; self.redraw()
    ...
    def add_folder_button(self):
        ...
        self.profile["pages"].append({"name": name, "wallpaper": "", "buttons": []}); b["folder"] = name
    ...
    def _create_linked_page(self, b):
        name = simpledialog.askstring("New page", "Page name:", parent=self, initialvalue=b.get("name", ""))
        if not name: return
        if name in [p["name"] for p in self.profile["pages"]]:
            messagebox.showerror("Name taken", "A page with that name already exists."); return
        self.snapshot()
        self.profile["pages"].append({"name": name, "wallpaper": "", "buttons": []}); b["folder"] = name; self.redraw()
    ...
    def enable_drop(self):
        if sys.version_info >= (3, 14):
            self.status.configure(text="Use 'Import app / shortcut' (drag/drop requires a Python 3.14 compatible tkinterdnd2)."); return
        if not TkinterDnD:
        ...
        try:
            ...
            self.bind("<<Drop>>", self.on_drop); self.status.configure(text="Drop .exe, .lnk, .url, or folders anywhere.")
        except Exception: self.status.configure(text="Drag/drop unavailable; use Import app / shortcut.")
    ...
    def add_app_paths(self, paths):
        ...
        for path in paths[:slots]:
            ...
            b = button(name); b.update(type=btn_type, action=target); self.current_page["buttons"].append(b); added.append(name)
        if added:
            self.selected = self.current_page["buttons"][-1]; self.redraw()
    ...
    def duplicate(self):
        if len(self.current_page["buttons"]) >= 8:
            messagebox.showinfo("Page full", "A page holds a maximum of 8 buttons."); return
        self.snapshot(); b = copy.deepcopy(self.selected); b["id"] = str(uuid.uuid4()); b["name"] += " Copy"; self.current_page["buttons"].append(b); self.selected = b; self.redraw()
    ...
    def profile_menu(self):
        ...
        if name:
            self.snapshot(); self.data["profiles"].append({"id": str(uuid.uuid4()), "name": name, "pages": [{"name": "Home", "wallpaper": "", "buttons": []}]})
            self.data["activeProfile"] = len(self.data["profiles"])-1; self.page = 0; self.redraw()
    ...
    def settings(self):
        ...
        theme = ctk.CTkOptionMenu(win, values=["Dark", "OLED Black", "Blue", "Purple", "Green"]); theme.set(self.data.get("theme", "Dark")); theme.pack(padx=25, pady=15)
        def save_settings():
            updated = {"deviceName": name.get(), "brightness": int(bright.get()), "theme": theme.get()}
            if any(self.data.get(key) != value for key, value in updated.items()):
                self.snapshot(); self.data.update(updated)
            win.destroy(); self.redraw()
        ctk.CTkButton(win, text="Save settings", command=save_settings).pack(pady=12)

    def new(self): self.data = project(); self.page = 0; self.selected = None; self.undo_stack = []; self.redo_stack = []; self._save_path = None; self._dirty = False; self._update_title(); self.redraw()
    ...
    def save_as(self):
        path = filedialog.asksaveasfilename(title="Save deck file", defaultextension=".deck",
            filetypes=[("CYD Deck", "*.deck"), ("JSON", "*.json")],
            initialfile=self._save_path.name if self._save_path else "my_deck.deck")
        if not path: return
        self._save_path = Path(path); self.save()
        ...
        if sys.platform == "win32":
            ...
            for i in range(26):
                ...
                if mask & (1 << i) and ctypes.windll.kernel32.GetDriveTypeW(root) == 2:
                    drives.append(root)
        if len(drives) == 1:
            if not messagebox.askyesno("Write SD card", f"Write deck.deck to {drives[0]}?"):
                return ""
            return drives[0]
    ...
    def load(self):
        ...
        try:
            ...
            self._dirty = False; self._update_title(); self.redraw()
