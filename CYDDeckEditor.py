...
class Editor(ctk.CTk):
    ...
    def make_ui(self):
        ...
        for text, command in [("New", self.new), ("Open", self.load), ("Save", self.save), ("Save As", self.save_as), ("Write SD card", self.export_sd), ("Undo", self.undo), ("Redo", self.redo)]:
            ctk.CTkButton(bar, text=text, width=105, command=command).pack(side="left", padx=4, pady=7)
        self.search = ctk.CTkEntry(bar, placeholder_text="Search buttons..."); self.search.pack(side="right", padx=8, pady=7)
    ...
    def draw_sidebar(self):
        ...
        for i, p in enumerate(self.profile["pages"]):
            row = ctk.CTkFrame(page_scroll, fg_color="transparent"); row.pack(fill="x", pady=2)
    ...
    def draw_grid(self):
        ...
        # Disable + Button when the page is already full
        ...
        if at_cap:
            ctk.CTkLabel(head, text="(page full — 8 max)", text_color="#888", font=ctk.CTkFont(size=11)).pack(side="right", padx=(0,6))
        ...
        items = [b for b in self.current_page["buttons"] if not term or term in b["name"].lower()]
        for i, b in enumerate(items[:8]):
            is_folder = b["type"] == "Folder"
            border_col = "#e6a817" if is_folder else None
            f = ctk.CTkFrame(grid, corner_radius=14, border_width=2,
                border_color=border_col if is_folder and b is not self.selected else ("#3b8ed0" if b is self.selected else "#2b2b2b"))
            ...
            l = ctk.CTkLabel(f, text=b["name"], wraplength=120, font=ctk.CTkFont(size=16, weight="bold"))
            l.pack(pady=(0,10)); l.bind("<Button-1>", lambda e, x=b: self.select(x))
            if is_folder:
    ...
    def draw_inspector(self):
        ...
        b = self.selected
        ctk.CTkLabel(self.inspector, text="BUTTON", font=ctk.CTkFont(weight="bold")).pack(pady=(14,4))
        ...
        def set_toggle():
            if b.get("toggle") != v.get():
                self.snapshot(); b["toggle"] = v.get()
        ctk.CTkCheckBox(self.inspector, text="Smart toggle", variable=v, command=set_toggle).pack(anchor="w", padx=12, pady=8)
    ...
    def _page_menu(self, idx):
        ...
        win = tk.Toplevel(self); win.overrideredirect(True); win.attributes("-topmost", True); win.configure(bg="#2b2b2b")
        ...
        ctk.CTkButton(win, text="↑  Move up",   width=160, **btn_cfg, state="normal" if idx > 0 else "disabled", command=lambda: action(lambda: self._move_page(idx, -1))).pack(fill="x", padx=4, pady=1)
        ctk.CTkButton(win, text="↓  Move down", width=160, **btn_cfg, state="normal" if idx < len(pages)-1 else "disabled", command=lambda: action(lambda: self._move_page(idx, +1))).pack(fill="x", padx=4, pady=1)
        ctk.CTkFrame(win, height=1, fg_color="#444").pack(fill="x", padx=6, pady=4)
        ctk.CTkButton(win, text="🗑  Delete", width=160, text_color="#e05555", anchor="w", fg_color="transparent", hover_color="#3a3a3a", corner_radius=4, state="normal" if len(pages) > 1 else "disabled", command=lambda: action(lambda: self._delete_page(idx))).pack(fill="x", padx=4, pady=(1,4))
        win.bind("<FocusOut>", lambda e: close()); win.focus_force()

    def _rename_page(self, idx):
        pages = self.profile["pages"]; old = pages[idx]["name"]
        ...
        for p in pages:
            for b in p.get("buttons", []):
                if b.get("type") == "Folder" and b.get("folder") == old: b["folder"] = new
        pages[idx]["name"] = new; self.redraw()
    ...
    def export_sd(self):
        ...
        if len(drives) == 1:
            # Always confirm even for a single drive — prevents writing to a USB stick by accident.
            if not messagebox.askyesno("Write SD card", f"Write deck.deck to {drives[0]}?"): return
            folder = drives[0]
        else:
            folder = filedialog.askdirectory(title="Choose the root of your SD card")
    ...
    def load(self):
        ...
        try:
            ...
            self._save_path = Path(path); self.page = 0; self.selected = None
